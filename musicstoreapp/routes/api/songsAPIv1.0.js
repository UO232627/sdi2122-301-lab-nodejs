    ...
    app.post("/api/v1.0/songs", function(req, res) {
        try{
            let song = {
                ...
                author: req.body.user //Ojo que no se si es user o author
            }
    ...
    app.put("/api/v1.0/songs/:id", function (req, res) {
        try{
            ...
            songsRepository.updateSong(song, filter, options).then(result => {
                ...
                //La _id NO existe o los datos enviados difieren de los ya almacenados.
        ...
        try{
            ...
            usersRepository.findUser(filter, options).then(user => {
                if (user === null){
                    ...
                    res.json({ message: "Usuario no autorizado", authenticated: false })
                }
                else{
                    let token = app.get('jwt').sign({ user: user.email, time: Date.now() / 1000}, "secreto");
                    res.status(200);
                    res.json({ message: "usuario autorizado", authenticated: true, token: token })
                }
            }).catch(error => {
                res.status(401);
                res.json({ message: "Se ha producido un error al verificar credenciales", authenticated: false })
            })
        } catch (e) {
            res.status(500);
            res.json({ message: "Se ha producido un error al verificar credenciales", authenticated: false })
